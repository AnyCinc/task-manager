...
async function loadCasesDashboard() {
  ...
  // 管理者: シンプルテーブル（FAX/架電/広告 × 面接数）
  let tFaxIv=0, tKadenIv=0, tHitoIv=0;
  const memberRows = data.byMember.map(m => {
    const fi=z(m.fax_interview), ki=z(m.kaden_interview), hi=z(m.hitokiwa_interview);
    const fTgt=getTarget(m.id,'FAX受電'), kTgt=getTarget(m.id,'架電バイト'), hTgt=getTarget(m.id,'ヒトキワ広告');
    tFaxIv+=fi; tKadenIv+=ki; tHitoIv+=hi;
    const color = caseAvatarColor(m.name);
    const cell = (v, tgt) => {
      const cls = tgt ? (v >= tgt ? 'color:#22c55e;font-weight:700' : '') : '';
      return `<td style="text-align:right;${cls}">${v}${tgt ? `<span style="font-size:9px;color:#94a3b8">/${tgt}</span>` : ''}</td>`;
    };
    return `<tr>
      <td><span class="member-avatar-xs" style="background:${color}">${esc(m.initial||m.name[0])}</span>${esc(m.name)}</td>
      ${cell(fi,fTgt)}${cell(ki,kTgt)}${cell(hi,hTgt)}
    </tr>`;
  }).join("");
  ...
  container.innerHTML = `<div class="cases-table-wrap" style="max-width:500px">
    <table class="cases-summary-table">
      <thead>
        <tr>
          <th></th>
          <th style="text-align:right;color:var(--fax-color)">FAX受電</th>
          <th style="text-align:right;color:var(--kaden-color)">架電バイト</th>
          <th style="text-align:right;color:var(--hitokiwa-color)">ヒトキワ広告</th>
        </tr>
        <tr>
          <th></th>
          <th style="text-align:right;font-size:0.78rem;font-weight:500;opacity:0.7">面接数</th>
          <th style="text-align:right;font-size:0.78rem;font-weight:500;opacity:0.7">面接数</th>
          <th style="text-align:right;font-size:0.78rem;font-weight:500;opacity:0.7">面接数</th>
        </tr>
      </thead>
      <tbody>
        <tr style="font-weight:700;border-bottom:2px solid #334155">
          <td>合計</td>
          <td style="text-align:right">${tFaxIv}</td>
          <td style="text-align:right">${tKadenIv}</td>
          <td style="text-align:right">${tHitoIv}</td>
        </tr>
        ${memberRows}
      </tbody>
    </table>
