...
function buildExtractSheet_(ss) {
  ...
  sh.getRange('C2').setFormula(
    '=ARRAYFORMULA(IF(B2:B="","",IFERROR(REGEXREPLACE(TRIM(B2:B),' + RE_TAIL_ALNUM + ',""),TRIM(B2:B))))'
  );
    ...
    '=ARRAYFORMULA(' +
      'IF(A2:A="","",' +
        'IF(REGEXMATCH(IFERROR(TRIM(B2:B)&"",""),' + RE_JA + '),' +
           'IFERROR(REGEXREPLACE(TRIM(B2:B),' + RE_TAIL_ALNUM + ',""),TRIM(B2:B)),' +
           'IF(REGEXMATCH(IFERROR(TRIM(C2:C)&"",""),' + RE_JA + '),' +
              'IFERROR(REGEXREPLACE(TRIM(C2:C),' + RE_TAIL_ALNUM + ',""),TRIM(C2:C)),' +
              '""))))'
  );
